    ...
    <>
      ...
      <main>
        ...
        >
...

          {
            ...
            <div className="sm:mb-8 sm:flex sm:justify-center mt-6">
              <div className="relative rounded-full py-1 px-3 text-sm leading-6 text-gray-600 ring-1 ring-gray-900/10 hover:ring-gray-900/20">
                For more studies, check out the following book 
          ...
          {/* {
            bibledata.biblebooks.map((data,index) => (
            <div key={index} className="mt-10">
              <h2 className=' mb-5 text-[2.5rem] font-bold leading-[4rem] tracking-tight text-yellow-900'>{data.label}</h2>
              <div  className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
              { data.options.map((book,bkIndex) => (
              <Card
              key={bkIndex}
              hoverable
              // style={{
              ...
              >
                <Meta title={book.label} description="www.instagram.com" />
              </Card>
              ))
              }</div>
             </div>
